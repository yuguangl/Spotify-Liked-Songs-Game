...
function handleAuthCallback() {
  ...
  if (error) {
    showError("setupError", "Spotify auth cancelled or failed: " + error);
    return;
  }

  if (!code) return; // No redirect yet — normal page load
  ...
  if (!verifier) {
    showError("setupError", "Auth error: verifier missing. Please try again.");
    return;
  }
  ...
  function fetchPage(url) {
    spotifyGet(url)
      ...
      .then(function(data) {
        ...
        // Only keep tracks that have a 30-second preview
...
function onTracksLoaded(collected) {
  if (collected.length < 3) {
    ...
    showError("setupError",
      "Not enough liked songs with previews found. " +
      "Add more songs to your Spotify library and try again."
    );
    return;
  }
...
function resetRoundUI() {
  ...
  // Album art (blurred until round ends)
...
function updateTimerUI() {
  var pct  = (state.timeLeft / TIMER_SECONDS) * 100;
  var fill = $("timerFill");
...
function showHint() {
  ...
  var hint = state.currentTrack.name
    .split(" ")
    .map(function(word) {
...
function isCorrectGuess(guess, title) {
  ...
  // Substring match (guess at least 3 chars)
  if (g.length >= 3 && t.indexOf(g) !== -1) return true;

  // Fuzzy match — allow up to 20% edit distance
